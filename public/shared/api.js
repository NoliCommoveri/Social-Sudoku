// The client half of identity: two calls, and one rule about what happens when
// the gate says no.
//
...
async function call(path, init) {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(detail ? String(detail) : `That did not work (${response.status}).`, response.status);
  }
  return body;
}
